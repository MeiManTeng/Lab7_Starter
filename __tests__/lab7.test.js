describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    expect(buttonText).toBe('Remove from Cart'); // ✅ This is the expected new state

    // Reset state: click again to restore initial state
    await button.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (const item of prodItems) {
      ...
      // Use evaluate to click inside the shadow DOM safely
    ...
    const countText = await cartCount.evaluate(node => node.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    //loop through every element to check that their buttons say "Remove from Cart"
  ...
  it('Checking number of items in cart on screen after removing from cart', async () => {
    ...
    for (const item of productItems) {
      ...
      await page.evaluate((shadowRoot) => {
        const button = shadowRoot.querySelector('button');
        if (button && button.innerText === 'Remove from Cart') {
          button.click();
        }
      }, shadowRootHandle);
    }
